...
local function getPriorityPlayer(priorityPlayers)
    -- Combine default priority players with any additional ones passed in
    local allPriorityPlayers = {}
    for _, name in ipairs(PRIORITY_PLAYERS) do
        table.insert(allPriorityPlayers, name)
    end
    ...
    for _, name in ipairs(allPriorityPlayers) do
...
local function getNearbyPrompt(maxDistance)
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("ProximityPrompt") and obj.Parent and obj.Parent:IsA("BasePart") then
            -- Check if the prompt's parent has a player ancestor
            local ancestor = obj.Parent
            while ancestor do
                if ancestor:IsA("Model") and ancestor:FindFirstChild("Humanoid") then
                    local playerObj = Players:GetPlayerFromCharacter(ancestor)
                    -- Only return prompt if it belongs to a priority player
                    if playerObj and table.find(PRIORITY_PLAYERS, playerObj.Name) then
...
local function giftPet(pet, target)
    -- Check if target is a priority player
    if not target or not target.Parent or not target.Character or not table.find(PRIORITY_PLAYERS, target.Name) then
        return false
    end
...
local function getNextTarget(priorityPlayers)
    ...
    local priority = getPriorityPlayer(allPriorityPlayers)
